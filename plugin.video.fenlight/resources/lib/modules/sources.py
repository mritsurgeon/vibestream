...
class Sources():
	...
	def get_sources(self):
		...
		logger('VibeStream get_sources', 'AFTER quality_preset filter count=%s (was %s)' % (len(filtered), len(results)))
		if not filtered and results:
			self._no_results_debug = {'raw_count': len(raw_results), 'orig_count': len(self.orig_results), 'scrapers': list(self.active_internal_scrapers) + (['external'] if self.active_external else []), 'reason': 'all_filtered_by_quality_preset'}
			return self._process_post_results()
		if not filtered:
			return self._process_post_results()
		if self.autoscrape:
