...
levels =['../../../..', '../../..', '../..', '..']
status_property_string = 'vibestream.download_status.%s'
...
def download_threads_manager(multi_downloads, image):
	...
	for item in multi_downloads:
		...
		set_property('vibestream.active_queued_downloads', json.dumps(remaining_downloads))
	clear_property('vibestream.active_queued_downloads')
...
class Downloader:
	...
	def get_active_downloads(self):
		return json.loads(get_property('vibestream.active_downloads') or '[]')
	...
	def add_active_download(self):
		...
		set_property('vibestream.active_downloads', json.dumps(active_downloads))
	...
	def clear_active_downloads(self):
		clear_property('vibestream.active_downloads')

	def set_percent_property(self, percent):
		set_property('vibestream.%s' % self.final_name, str(percent))
