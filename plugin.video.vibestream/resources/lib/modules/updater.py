...
destination_check = translate_path('special://home/addons/plugin.video.vibestream/')
changelog_location = translate_path('special://home/addons/plugin.video.vibestream/resources/text/changelog.txt')
downloads_icon = get_icon('downloads')
addon_dir = 'plugin.video.vibestream'
zipfile_name = 'plugin.video.vibestream-%s.zip'
heading_str = 'VibeStream Updater'
error_str = 'Error'
notification_occuring_str = 'VibeStream Update Occuring'
notification_available_str = 'VibeStream Update Available'
notification_updating_str = 'VibeStream Performing Update'
notification_rollback_str = 'VibeStream Performing Rollback'
...
success_str = '[CR]Success.[CR]VibeStream updated to version [B]%s[/B]'
rollback_heading_str = 'Choose Rollback Version'
success_rollback_str = '[CR]Success.[CR]VibeStream rolled back to version [B]%s[/B]'
confirm_rollback_str = 'Are you sure?[CR]Version [B]%s[/B] will overwrite your current installed version.' \
						'[CR]VibeStream will set your update action to [B]OFF[/B] if rollback is successful'
...
no_changes_str = 'You are running the current version of VibeStream.[CR][CR]There is no new version changelog to view.'

def get_location(insert=''):
	return 'https://github.com/%s/%s/raw/main/packages/%s' % (get_setting('vibestream.update.username'), get_setting('update.location'), insert)
...
def rollback_check():
	...
	url = 'https://api.github.com/repos/%s/%s/contents/packages' % (get_setting('vibestream.update.username'), get_setting('update.location'))
	show_busy_dialog()
	results = requests.get(url)
	hide_busy_dialog()
	if results.status_code != 200: return ok_dialog(heading=heading_str, text=error_rollback_str)
	results = [i['name'].split('-')[1].replace('.zip', '') for i in results.json() if 'plugin.video.vibestream' in i['name'] \
