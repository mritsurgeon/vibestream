...
int_window_prop = 'vibestream.internal_results.%s'
scraper_timeout = 25
...
class Sources():
	...
	def playback_prep(self, params=None):
		...
		self.folders_ignore_filters = get_setting('vibestream.results.folders_ignore_filters', 'false') == 'true'
		self.filter_size_method = int(get_setting('vibestream.results.filter_size_method', '0'))
		...
		self.include_unknown_size = get_setting('vibestream.results.size_unknown', 'false') == 'true'
		self.qm = QualityManager()
		size_direction_override = 1 if self.qm.preset.get('prefer_smallest') else None
		self.sort_function, self.quality_filter = results_sort_order(size_direction_override), self._quality_filter()
	...
	def get_sources(self):
		qm = getattr(self, 'qm', QualityManager())
		...
		# 3. Process all results, then apply quality preset filter (show only sizes within selected preset)
		if not raw_results:
			self.orig_results = []
		...
		if not results:
			...
			return self._process_post_results()
		# 4. Apply quality preset filter (resolution + size) so only results within selected preset are shown
		filtered = qm.filter_sources(results)
		filtered_count = len(results) - len(filtered)
		logger('VibeStream get_sources', 'AFTER quality_preset filter: shown=%d, hidden=%d (was %d)' % (len(filtered), filtered_count, len(results)))
		
		if not filtered and results:
			self._no_results_debug = {
				'raw_count': len(raw_results), 
				'orig_count': len(results), 
				'filtered_count': filtered_count,
				'scrapers': list(self.active_internal_scrapers) + (['external'] if self.active_external else []), 
				'reason': 'all_hidden_by_quality_preset'
			}
			return self._process_post_results()
		...
		if self.autoscrape:
			return filtered
		return self.play_source(filtered)
		...
		if self.active_external or self.background:
			if self.active_external:
				if any((i[0] == 'comet' for i in self.external_providers)):
					...
					debrid_token = get_setting('vibestream.%s' % debrid_token_dict[debrid_service])
	...
	def filter_results(self, results):
		...
		if self.filter_size_method:
			min_size = string_to_float(get_setting('vibestream.results.%s_size_min' % self.media_type, '0'), '0') / 1000
			...
			if self.filter_size_method == 1:
				...
				max_size = string_to_float(get_setting('vibestream.results.%s_size_max' % self.media_type, '10000'), '10000') / 1000
	...
	def special_filter(self, results, file_type):
		...
		if key == '[B]HEVC[/B]' and enable_setting == 0:
			hevc_max_quality = self._get_quality_rank(get_setting('vibestream.filter.hevc.%s' % ('max_autoplay_quality' if self.autoplay else 'max_quality'), '4K'))
	...
	def get_folderscraper_info(self):
		folder_info = [(get_setting('vibestream.%s.display_name' % i), i, source_folders_directory(self.media_type, i)) for i in folder_scrapers]
	...
	def _process_post_results(self):
		...
		if orig and not self.background:
			...
			filtered_out = debug.get('filtered_count', len(orig))
			
			if reason == 'all_hidden_by_quality_preset':
				msg = 'No results found under your [B]%s[/B] quality preset.[CR](%d results were hidden).[CR][CR]Would you like to see all results?' % (qm.active_preset_id.replace('_', ' ').title(), filtered_out)
			else:
				msg = 'No results found after filtering.[CR]Access %d filtered results?' % len(orig)
			
			if confirm_dialog(heading='No Results Found', text=msg):
				return self._process_ignore_filters()
	...
	def debridPacks(self, debrid_provider, name, magnet_url, info_hash, download=False):
		...
		self._kill_progress_dialog()
		return VibeStreamPlayer().run(link, 'video')
