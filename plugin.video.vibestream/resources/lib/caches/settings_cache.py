...
def get_setting(setting_id, fallback=''):
	# Try memory first with either prefix
...
def set_boolean(params):
	...
	set_setting(setting, boolean_dict[get_setting('vibestream.%s' % setting)])

def set_string(params):
	current_value = get_setting('vibestream.%s' % params['setting_id'])
...
def set_path(params):
	...
	new_value = kodi_dialog().browse(browse_mode, '', '', defaultt=get_setting('vibestream.%s' % setting_id))
	set_setting(setting_id, new_value)
...
def set_source_folder_path(params):
	setting_id = params['setting_id']
	current_setting = get_setting('vibestream.%s' % setting_id)
...
#===============================================================================#
#==================== General
{'setting_id': 'auto_start_vibestream', 'setting_type': 'boolean', 'setting_default': 'false'},
...
#==================== Downloads
{'setting_id': 'movie_download_directory', 'setting_type': 'path', 'setting_default': 'special://profile/addon_data/plugin.video.vibestream/Movies Downloads/', 'browse_mode': '0'},
{'setting_id': 'tvshow_download_directory', 'setting_type': 'path', 'setting_default': 'special://profile/addon_data/plugin.video.vibestream/TV Show Downloads/', 'browse_mode': '0'},
{'setting_id': 'premium_download_directory', 'setting_type': 'path', 'setting_default': 'special://profile/addon_data/plugin.video.vibestream/Premium Downloads/', 'browse_mode': '0'},
{'setting_id': 'image_download_directory', 'setting_type': 'path', 'setting_default': 'special://profile/addon_data/plugin.video.vibestream/Image Downloads/', 'browse_mode': '0'},
...
# Main menus (root, Movies list, TV list) = wide list (55). Sub menus with titles = poster (500), not wall.
...
#==================== External (CocoScrapers enabled by default when script.module.cocoscrapers is installed)
