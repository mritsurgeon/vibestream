...
def clear_icons():
    ...
    try:
        ...
        icon = dbcur.execute("SELECT cachedurl FROM texture WHERE url ='" + addonPath + "/plugin.video.vibestream/resources/media/vibestream_icon.png';").fetchone()
        fanart = dbcur.execute("SELECT cachedurl FROM texture WHERE url ='" + addonPath + "/plugin.video.vibestream/resources/media/vibestream_fanart.png';").fetchone()
        
        if icon is not None:
            if xbmcvfs.exists(thumbnailsPath + icon[0]):
                xbmcvfs.delete(thumbnailsPath + icon[0])
            dbcur.execute("DELETE FROM texture WHERE url ='" + addonPath + "/plugin.video.vibestream/resources/media/vibestream_icon.png';")
        
        if fanart is not None:
            if xbmcvfs.exists(thumbnailsPath + fanart[0]):
                xbmcvfs.delete(thumbnailsPath + fanart[0])
            dbcur.execute("DELETE FROM texture WHERE url ='" + addonPath + "/plugin.video.vibestream/resources/media/vibestream_fanart.png';")
