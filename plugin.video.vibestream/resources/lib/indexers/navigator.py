...
class Navigator:
	...
	def tools(self):
		...
		if get_setting('vibestream.external_scraper.module') not in ('empty_setting', ''):
		...
		if get_setting('vibestream.use_viewtypes', 'true') == 'true' and not get_setting('vibestream.manual_viewtypes', 'false') == 'true':
	...
	def changelog_utils(self):
		vibestream_clogpath = tp('special://home/addons/plugin.video.vibestream/resources/text/changelog.txt')
		self.add({'mode': 'show_text', 'heading': 'Changelog', 'file': vibestream_clogpath, 'font_size': 'large', 'isFolder': 'false'}, 'Changelog', 'lists')
	...
	def exit_media_menu(self):
		params = get_property('vibestream.exit_params')
		if params: return container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.vibestream/resources/text/tips'
