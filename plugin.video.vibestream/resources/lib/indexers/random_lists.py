...
movie_main = ('tmdb_movies_popular', 'tmdb_movies_popular_today', 'tmdb_trending_movie_week', 'tmdb_movies_blockbusters','tmdb_movies_in_theaters', 'tmdb_movies_upcoming', 'tmdb_movies_latest_releases',
...
tvshow_main = ('tmdb_tv_popular', 'tmdb_tv_popular_today', 'tmdb_trending_tv_week', 'tmdb_tv_premieres', 'tmdb_tv_airing_today','tmdb_tv_on_the_air','tmdb_tv_upcoming',
...
class RandomLists():
	def __init__(self, params):
		...
		self.remake_widgets = self.is_external and get_property('vibestream.refresh_widgets') == 'true'
	...
	def make_directory(self, next_page_params={}):
		...
		if self.is_external:
			if self.folder_name: set_property('vibestream.%s' % self.folder_name, self.category_name)
			else: set_property('vibestream.%s' % self.base_list_name, self.category_name)
...
def random_shortcut_folders(folder_name, random_results):
	...
	remake_widgets = is_external and get_property('vibestream.refresh_widgets') == 'true'
	...
	if is_external: set_property('vibestream.%s' % folder_name, list_name)
