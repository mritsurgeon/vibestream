...
current_skin_prop, current_font_prop = 'vibestream.current_skin', 'vibestream.current_font'
addon_skins_folder = 'special://home/addons/plugin.video.vibestream/resources/skins/Default/1080i/'
...
def window_manager(obj):
	...
	def monitor():
		timer = 0
		while not get_property('vibestream.window_loaded') == 'true' and timer <= 5:
			sleep(50)
			timer += 0.05
		hide_busy_dialog()
		obj.close()
	...
	def add_to_stack(params):
		...
		set_property('vibestream.window_stack', json.dumps(window_stack))
...
class BaseDialog(window_xml_dialog):
	...
	def set_home_property(self, prop, value):
		set_property('vibestream.%s' % prop, value)

	def get_home_property(self, prop):
		return get_property('vibestream.%s' % prop)

	def clear_home_property(self, prop):
		return clear_property('vibestream.%s' % prop)
...
class FontUtils:
	...
	def match_font(self, size, bold, fallback):
		font_tag = 'VIBESTREAM_%s%s' % (size, '_BOLD' if bold else '')
