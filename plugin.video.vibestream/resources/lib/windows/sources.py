...
class SourcesResults(BaseDialog):
	...
	def filter_action(self, action):
		...
		if action in self.selection_actions:
			...
			if filter_type in ('quality', 'provider'):
				...
				elif filter_value == 'ignore_filters':
					self.selected = ('ignore_filters', '')
					return self.close()
	...
	def make_filter_items(self):
		...
		if not self.filters_ignored: data.append(('Show [B]All[/B] (Ignore Quality Filters)', 'special', 'ignore_filters'))
