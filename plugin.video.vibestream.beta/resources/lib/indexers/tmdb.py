	...
	try:
		...
		if shuffle:
			...
			if returning_to_list:
				try: 
					lists = json.loads(get_property('vibestream.tmdb.lists.order'))
				except Exception as e: 
					kodi_utils.logger('TMDB ERROR', f'Failed to load lists from property: {str(e)}')
			else:
				try:
					random.shuffle(lists)
					set_property('vibestream.tmdb.lists.order', json.dumps(lists))
				except Exception as e:
					kodi_utils.logger('TMDB ERROR', f'Error in shuffle: {str(e)}')
			
			sort_method = 'none'
		else:
			clear_property('vibestream.tmdb.lists.order')
...
def build_tmdb_list(params):
	...
	try:
		...
		if page_no == 1 and not is_external: set_property('vibestream.exit_params', folder_path())
