...
season_prop, media_prop = 'vibestream.meta_season_%s', 'vibestream.%s_%s_%s'
...
class MetaCache:
		...
		try:
			...
			if meta is None:
				...
				if cache_data is not None and len(cache_data) >= 2 and cache_data[0] is not None:
					meta, expiry = json.loads(cache_data[0]), cache_data[1]
	...
	def set(self, media_type, id_type, meta, expiration=168, current_time=None):
		try:
			...
			dbcon.execute(SET_MOVIE_SHOW, (media_type, string(meta_get('tmdb_id')), meta_get('imdb_id'), string(meta_get('tvdb_id')), json.dumps(meta), expires))
	...
	def set_season(self, prop_string, meta, expiration=168):
		try:
			...
			dbcon.execute(SET_SEASON, (prop_string, json.dumps(meta), int(expires)))
		except Exception: return None
	...
	def set_memory_cache_season(self, prop_string, meta, expires):
		try:
			...
			set_property(season_prop % prop_string, json.dumps(cachedata))
		except Exception: pass
	...
	def get_function(self, prop_string):
		...
		try:
			...
			if cache_data is not None and len(cache_data) >= 3 and cache_data[1] is not None:
				if cache_data[2] > current_time: result = json.loads(cache_data[1])
	...
	def set_function(self, prop_string, result, expiration=24):
		try:
			dbcon = connect_database('metacache_db')
			expires = get_timestamp(expiration)
			dbcon.execute(SET_FUNCTION, (prop_string, json.dumps(result), expires))
		except Exception: return
